...
public final class AppViewModel: ObservableObject {
    ...
    private func updateFilteredChannels() async {
        ...
        case .favorites:
            ...
            return
        case .history:
            // Оптимизация: если поиска нет, получаем каналы напрямую за O(N) от размера истории (max 50)
            if searchQuery.isEmpty {
                self.filteredChannels = await filterEngine.getChannels(ids: historyIds)
            } else {
                // Если есть поиск, фильтруем только те каналы, которые есть в истории
                let matchingHistory = await filterEngine.filter(
                    query: searchQuery,
                    category: nil,
                    country: nil,
                    language: nil,
                ...
                // Чтобы сохранить хронологический порядок истории при поиске, используем Map для O(1) lookup
                let channelMap = matchingHistory.reduce(into: [String: Channel](minimumCapacity: matchingHistory.count)) { map, channel in
