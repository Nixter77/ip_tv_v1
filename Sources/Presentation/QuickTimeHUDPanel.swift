...
struct AutoHideHUDModifier: ViewModifier {
    ...
    private func scheduleShow() {
        hideTimer?.invalidate()
        if isCursorHidden { NSCursor.unhide(); isCursorHidden = false }
        if !isVisible {
    ...
    private func scheduleHide() {
        ...
        if !isCursorHidden { NSCursor.hide(); isCursorHidden = true }
    }
}
...
private final class Coordinator: NSObject {
    ...
    override init() {
        super.init()
        monitor = NSEvent.addLocalMonitorForEvents(matching: [.mouseMoved, .mouseEntered, .leftMouseDown, .rightMouseDown]) { [weak self] event in
            self?.onMouseMoved?()
            return event
        }
    }
...
public struct QuickTimeHUDPanel: View {
        ...
        VStack(spacing: 10) {
            ...
            HStack(spacing: 12) {
                ...
                if let bitrateBinding = preferredBitrate {
                    ...
                    .frame(width: 64)
                    .scaleEffect(0.85)
                    .accessibilityLabel("Выбор битрейта")
                }
    ...
    private var timelineView: some View {
        VStack(spacing: 4) {
            ...
            .controlSize(.mini)
            .tint(.white)
            .accessibilityLabel("Полоса прокрутки")
