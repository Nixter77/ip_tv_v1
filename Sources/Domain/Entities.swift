...
public struct Stream: Decodable, Equatable, Hashable, Sendable {
    ...
    /// Regex to identify sensitive key-value parameters following common delimiters (?, &, /, |, ;, #)
    private static let paramRegex: NSRegularExpression = {
        let pattern = #"(?<=[?&/|;#])([^?&/|;=\s#]+)=[^?&/|;#\s]+"#
        return try! NSRegularExpression(pattern: pattern)
    }()

    /// Regex to find URLs in unstructured text with better boundary detection
    ...
    public static func mask(_ urlString: String) -> String {
        ...
        // Secondary pass: Mask key=value patterns in the whole string (covers paths and custom delimiters)
        let fullRange = NSRange(result.startIndex..<result.endIndex, in: result)
        result = paramRegex.stringByReplacingMatches(in: result, range: fullRange, withTemplate: "$1=****")
