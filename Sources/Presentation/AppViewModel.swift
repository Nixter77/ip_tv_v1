...
public final class AppViewModel: ObservableObject {
    ...
    private func updateFilteredChannels() async {
        ...
        case .language(let code):
            languageFilter = code
        case .favorites:
            // Оптимизация: используем Subset Pruning в FilterEngine, передавая favoriteIds.
            // Это позволяет движку сузить область поиска до избранных каналов в самом начале,
            // что значительно быстрее, чем фильтрация полного списка результатов в памяти ViewModel.
            ...
            return
        case .history:
            // Оптимизация: используем Subset Pruning в FilterEngine, передавая историю ID.
            // Движок вернет только те каналы из истории, которые соответствуют поисковому запросу.
            let matchingHistoryChannels = await filterEngine.filter(
                query: searchQuery,
                category: nil,
                country: nil,
                language: nil,
            ...
            // Поскольку FilterEngine возвращает каналы отсортированными по имени (allChannelsSorted.filter),
            // нам нужно восстановить оригинальный хронологический порядок истории.
            let channelMap = matchingHistoryChannels.reduce(into: [String: Channel](minimumCapacity: matchingHistoryChannels.count)) { map, channel in
