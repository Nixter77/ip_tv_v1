...
public protocol ChannelFilterEngineProtocol: Sendable {
    ...
    ///   - matchingIds: Опциональное подмножество ID каналов для фильтрации (например, избранное)
    func filter(
        query: String?,
        category: String?,
        country: String?,
...
/// Расширение для обеспечения обратной совместимости при вызове без новых параметров
...
public actor ChannelFilterEngine: ChannelFilterEngineProtocol {
    ...
    public func filter(
        ...
        // Если результат маленький (< 1000), быстрее собрать его по ID и отсортировать O(M log M).
        // Если результат большой, быстрее пройтись по всему отсортированному списку O(N).
        if finalIds.count < 1000 {
            return finalIds.compactMap { self.channels[$0] }.sorted { $0.name < $1.name }
        } else {
