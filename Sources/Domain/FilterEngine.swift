...
public extension ChannelFilterEngineProtocol {
    func filter(
        ...
        await self.filter(query: query, category: category, country: country, language: language, matchingIds: matchingIds)
    }
}
...
public actor ChannelFilterEngine: ChannelFilterEngineProtocol {
    ...
    /// Получить список каналов по их ID (сохраняя порядок переданного массива)
