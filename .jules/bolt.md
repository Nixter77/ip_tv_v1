...
## 2026-06-18 - [Optimization] Subset Pruning and Adaptive Sorting
**Learning:** For engines managing large datasets (50k+ items), global filtering is expensive. Introducing "Subset Pruning" (passing a restricted set of IDs like Favorites/History to the engine) reduces search space to O(M). Additionally, an "Adaptive Sorting" strategy—switching between O(M log M) dictionary lookups for small sets and O(N) filtering of a pre-sorted master list for large sets—ensures the engine remains fast regardless of result density.
**Action:** Implement `matchingIds` in filter protocols and use result-size thresholds to choose the fastest sorting strategy.
