...
/// Расширение протокола для поддержки дефолтных значений в методе filter
...
public actor ChannelFilterEngine: ChannelFilterEngineProtocol {
    ...
    ///   - matchingIds: Опциональный набор ID для предварительной фильтрации (например, Избранное)
