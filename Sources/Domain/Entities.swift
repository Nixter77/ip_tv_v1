...
public struct Stream: Decodable, Equatable, Hashable, Sendable {
    ...
    public static func mask(_ urlString: String) -> String {
        ...
        guard var components = components, components.scheme != nil else {
            // Fail-secure: If parsing fails even after encoding, try a robust regex-based mask
            // for both credentials and common sensitive parameter patterns.
            let maskedCredentials = urlString.replacingOccurrences(of: "://[^@]+@", with: "://****@", options: .regularExpression)
            let parameterPattern = #"(?<=[?&/|;])([^?&/|;=\s#]+)=[^?&/|;\s#]+"#
            return maskedCredentials.replacingOccurrences(of: parameterPattern, with: "$1=****", options: .regularExpression)
        }
